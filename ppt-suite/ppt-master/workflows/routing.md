---
description: Deterministic selection among PPT Master's five top-level artifact routes.
---
...
| Rule | Behavior |
|---|---|
| One artifact lifecycle | Every request enters Generate PPTX, Create Template, Fill Native PPTX, Enhance Native PPTX, or Full-slide Image |
...
| Full-slide Image | Deliver AI-generated 16:9 full-slide PNGs (or an image-only PPTX) with no object-level editability commitment | Hand off to the `rw-consulting-ppt` skill (Production Brief → Prompt Assembly → Sample Gate → Batch Plan → image_gen → Contact Sheet → PNG management → Image-only PPTX Packaging). This is a coordinated route, not a child workflow of this package | User explicitly requests full-slide images / image-only delivery; editable data-backed charts not required | Generate full-slide PNGs; package each page as one full-slide image in an image-only PPTX | Full-slide PNGs + optional image-only PPTX; passes `ppt-production-qa` image-only mode |
