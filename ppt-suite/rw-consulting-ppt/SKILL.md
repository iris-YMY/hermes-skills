...
- **Route ownership (2026-08-08)**: this skill is the implementation of the `Full-slide Image` top-level route in `ppt-master`'s routing matrix (production brief → prompt assembly → sample gate → batch plan → image_gen → contact sheet → PNG management → image-only PPTX packaging). It is also the target of `ppt-router`'s Production-Format decision and the handoff destination of `consulting-deck-strategist` for full-slide-image decks. Final output passes `ppt-production-qa` (image-only mode) before delivery.
...
- This image-only route is explicitly exempt from native-chart and embedded-workbook requirements. If the user requires editable data-backed charts, switch to `ppt-master` instead of weakening this routing lock.
...
7. Page outline: user-provided page-by-page outline, or permission for the agent to propose one.
...
- evidence boundary: confirmed facts, user assumptions, agent inference, and open uncertainties;
...
- 渲染/检查工具：vision_analyze（生成图稿视觉 QA）、soffice（pptx 转 PDF/PNG 预览）
- 格式库：python-pptx（可选 PPTX 打包）、Pillow（图片处理）
- 相关技能：ppt-workflow（四阶段流程总控）、powerpoint（可编辑 pptx 还原）、presentation-visual-director（视觉指导）
- 图片生成：Hermes 下使用 image_gen 工具集或 comfyui skill 替代原 Gemini 后端
