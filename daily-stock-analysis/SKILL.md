...
- **Multi-market**: A-shares (CN), HK stocks, US stocks + indices (SPX, DJI, IXIC), **China mutual funds** (基金)
- **Data sources**: AkShare, Tushare, YFinance for quotes; Tavily/SerpAPI/Brave for news; East Money API for fund NAV (see `references/fund-apis.md`)
...
## Deployment Status (CONFIRMED 2026-06-24)

⚠️ **This skill is a reference architecture, NOT deployed on this server.**
- Repository clone fails (GitHub network timeout from server)
- Python dependencies (akshare, litellm, etc.) not installed
- No LLM keys or notification channels configured

**Lightweight alternative (validated)**: Use 东方财富 API directly with Python `urllib` (zero dependencies) + 千问 model + Hermes cron job for fund/stock monitoring. See `references/eastmoney-fund-api.md` for API details and verified code.

**For fund analysis specifically**:
- Fund NAV history: `api.fund.eastmoney.com/f10/lsjz` ✅ verified for 8 funds
- Fund holdings: `fund.eastmoney.com/pingzhongdata/{code}.js` ✅ verified
- Real-time estimates: ❌ not accessible from server (T+1 only)
- Use Hermes cron job (pre-fetch script pattern from broadcast-cronjobs) instead of full deployment
...
## ⚠️ Deployment Status (updated 2026-06-24)

**Current state: Documentation-only. NOT installed or runnable.**

The skill was added from reading the GitHub repo docs, but the actual codebase has never been deployed. Key blockers:

| Blocker | Detail | Status |
|---------|--------|--------|
| GitHub clone | `git clone` times out from server (GitHub network issues) | ❌ |
| Python deps | `pip install akshare` blocked by PEP 668 (externally-managed-environment) | ❌ |
| AkShare/efinance/tushare | None installed — A-share data layer non-functional | ❌ |
| litellm | Not installed — LLM analysis layer non-functional | ❌ |
| wkhtmltopdf | System tool not installed — report-to-image broken | ❌ |
| News API keys | Tavily/SerpAPI not configured | ❌ |

**Minimum to actually run**: LLM key (Gemini free tier) + 1 notification channel (Feishu webhook) + clone repo + install deps in venv.

### Recommended Lightweight Alternative

Instead of deploying the full repo, build fund/stock analysis as **Hermes native cron job** with pre-fetch script pattern (same as broadcast-cronjobs §9):

```
┌──────────────┐     ┌─────────────────────┐     ┌──────────────┐
│  Hermes cron  │────▶│ fund_data_fetcher.py │────▶│  qwen model  │
│  (scheduler)  │     │ (东方财富 API, zero   │     │  (formatting │
│               │     │  deps, urllib only)  │     │   + analysis)│
└──────────────┘     └─────────────────────┘     └──────────────┘
```

**Advantages**: 0 extra dependencies, 0 extra LLM keys, 0 network issues, uses existing Feishu delivery.

**Verified APIs** (see `references/fund-apis.md`):
- `api.fund.eastmoney.com/f10/lsjz` — fund NAV history (8/8 funds tested OK)
- `fund.eastmoney.com/pingzhongdata/{code}.js` — fund info + holdings
- `qt.gtimg.cn` — real-time stock prices (for fund estimate)
...
# China mutual funds (基金, 6-digit code)
008586    # 华夏人工智能ETF联接C
110017    # 易方达增强回报债券A
000218    # 国泰黄金ETF联接A
# Fund data via East Money API — see references/fund-apis.md
# ⚠️ Fund NAV is T+1 (no real-time), no K-line, analysis focuses on hold/add/reduce
```
