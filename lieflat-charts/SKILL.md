...
> **Hermes 环境适配**：本机为完整仓库（124 文件，含 templates/reports 与 maps-gallery）。安装/渲染验证（IntersectionObserver 懒加载坑）与 PPT 融合用法见 `references/hermes-verification.md`。
...
> ⚠️ **Hermes 无头浏览器验证陷阱（2026-08 实测）**：gallery 和成品图用 `obsReveal`（IntersectionObserver，threshold 0.3）懒加载——元素滚入视野才渲染 SVG。在无头浏览器中**快速滚动或直接截图会看到空白/半成品图表**，容易误判"skill 渲染不到位"。正确验证流程：
> 1. `browser_console` 检查是否真渲染：`Array.from(document.querySelectorAll('svg')).map(s=>s.innerHTML.length)`（0 = 未渲染）。
> 2. 触发全部渲染（最可靠）：对每个 svg 模拟 click（页面注册了点击重播）：`document.querySelectorAll('svg').forEach(s=>s.dispatchEvent(new MouseEvent('click',{bubbles:true})))`。
> 3. 确认所有 svg innerHTML 非空后再截图汇报，不要把首屏半成品当最终效果。
